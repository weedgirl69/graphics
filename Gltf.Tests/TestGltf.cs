namespace Gltf.Tests {
  ...
  public class TestGltf {
    ...
    public TestGltf(ITestOutputHelper testOutputHelper) => this._testOutputHelper = testOutputHelper;

    [Theory]
    [InlineData("%GLTF_SAMPLE_MODELS_DIR%/2.0")]
    private async Task TestModels(string directoryExpression) {
      var directory = this._testOutputHelper.Log(formatString: "directory={0}",
      ...
      await Task.WhenAll(from gltfPath in gltfFilesEnumerator
                         select TestGltf.ValidateModel(gltfPath));
    }
