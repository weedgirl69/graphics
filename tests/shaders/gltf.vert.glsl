...
layout(binding = 0) uniform FrameUniforms { mat4 view_projection; };

void main() {
  float far = 1000;
  float near = .001;
  vec3 position = vec4(in_position * .01, 1.0) * in_instance_transform;
  mat4 projection = mat4(1.5, 0, 0, 0, 0, -1.5, 0, 0, 0, 0, far / (near - far),
                         (near * far) / (near - far), 0, 0, -1, 0);
  vec3 camera_position = vec3(0, 1, 2);
  ...
  mat4 view = mat4(vec4(right, -dot(right, camera_position)),
