...
float visibility_occlusion(float alpha_roughness,float n_dot_l,float n_dot_v){
    float alpha_roughness_sq=alpha_roughness*alpha_roughness;
    
    float GGXV=n_dot_l*sqrt(n_dot_v*n_dot_v*(1.-alpha_roughness_sq)+alpha_roughness_sq);
    float GGXL=n_dot_v*sqrt(n_dot_l*n_dot_l*(1.-alpha_roughness_sq)+alpha_roughness_sq);
...
vec3 specular_reflection(float v_dot_h,vec3 reflectance_0,vec3 reflectance_90){
    return reflectance_0+(reflectance_90-reflectance_0)*pow(1.-v_dot_h,5.);
}
...
void main(){
    ...
    if(n_dot_l>0.||n_dot_v>0.){
        vec3 light_contribution=4.*n_dot_l*(diffuse_contribution+specular_contribution);
